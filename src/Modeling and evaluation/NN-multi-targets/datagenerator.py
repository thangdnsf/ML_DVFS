...
def AddGaussianNoise(tensor, mean=0., std=0.01):
    return tensor + torch.rand(tensor.size())*torch.Tensor(tensor.size()).random_(-1,2)*tensor* std + mean
...
class HPC_DVFS(Dataset):
    def __init__(self, df, mode,feature_list = None, sc = None, isscfit = False,transform=None, augmentation = True):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'test':
            ...
            if self.augmentation:
                return AddGaussianNoise(torch.tensor(inputarr),std=random.randrange(0, 10)/1000).float(), label
            else:
                return torch.tensor(inputarr).float(), label